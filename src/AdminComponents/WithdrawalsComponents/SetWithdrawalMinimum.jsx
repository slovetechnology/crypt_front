...
const SetWithdrawalMinimum = ({ closeView }) => {
    ...
    return (
        <ModalLayout closeView={closeView} toggler={toggler}>
            <div className='xl:w-1/3 lg:w-2/5 md:w-1/2 w-10/12 h-fit bg-white rounded-lg overflow-hidden relative' ref={toggler}>
                {loading && <Loading />}
                <div className='flex flex-col gap-4 py-6 px-2'>
                    <div className='md:text-xl text-lg uppercase text-center font-bold border-b w-full mb-2'>set withdrawal minimum</div>
                    <div className='flex flex-col gap-2 items-center text-sm'>
                        <div className='text-center'>Enter minimum withdrawal amount:</div>
