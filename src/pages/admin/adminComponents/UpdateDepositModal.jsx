...
const UpdateDepositModal = ({ closeView, singleDeposit, setAllDeposits, setStart, setEnd, setpagestart, setpageend, setSearch, setWrite, refetchAllDeposits }) => {
                          ...
                          {statuses.map((item, i) => (
                            <div className='flex flex-col mt-2' key={i}>
                              <div className='flex items-center cursor-pointer hover:bg-[#e6e5e5]' onClick={() => { setStatus(item.status); setStatusShow(false); setUpdate(true) }}>
                                <div className={`text-[0.85rem] font-bold ${item.status === 'confirmed' && 'text-[#b19e32]'} ${item.status === 'failed' && 'text-[red]'}`}>{item.status}</div>
                              </div>
                            </div>
                          ))}
                        </div>}
