...
const BuyPlanModal = ({ setModal, buybal, setToggle, userDeposits, refetchDeposits, refetchWallet, refetchInvestments, refetchInvestmentsUnclaim, refetchNotifications, refetchUnreadNotis }) => {
                ...
                <FaXmark className='absolute top-0 right-1 cursor-pointer text-2xl' onClick={() => setModal(false)} />
                    ...
                    <div className='relative flex gap-3 items-center mx-auto'>
                        <div className='relative'>
                            <input className={`outline-none border lg:text-[0.85rem] w-full h-8 rounded-[5px] px-5 bg-transparent ipt ${error === 'amount' ? 'border-[red]' : 'border-[#5BB4FD]'}`} value={amount} onChange={e => setAmount(e.target.value)} placeholder='Enter Amount'></input>
                            <div className='absolute top-1.5 left-2 text-[0.85rem]'>$</div>
                        </div>
                        <div className={`h-fit w-fit text-nowrap py-2 md:px-4 px-2 ${error === 'balance' ? 'border border-[red]' : ''} bg-[#5BB4FD] flex flex-col gap-1 items-center justify-center text-white text-[0.85rem] rounded-md`}>
